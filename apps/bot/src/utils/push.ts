 ...
 * Metadados do evento.
 *
 * Serviam à deduplicação do LBS Notify, descontinuado em 19/09/2026. Ficam no
 * contrato porque documentam QUAL fato gerou o aviso — e, sem ninguém para
 * deduplicar, o `eventId` passou a ser ainda mais importante de acertar: derive
 * do fato (tarefa + instante do lembrete), nunca de `Date.now()`.
 */
...
export async function sendPushToUser(
  userId: string,
  payload: PushPayload,
  // Mantido no contrato por quem chama; nao e usado desde que a central saiu.
  _meta?: MetaEvento,
): Promise<void> {
