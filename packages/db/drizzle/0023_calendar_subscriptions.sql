-- Calendários externos (.ics / webcal) assinados pelo usuário. Cada ocorrência
-- do feed vira uma tarefa comum, marcada com source='ics' + calendar_id.
CREATE TABLE IF NOT EXISTS "calendar_subscriptions" (
	...
	"group_id" varchar(8) REFERENCES "task_groups"("id") ON DELETE SET NULL,
...
ALTER TABLE "tasks" ADD COLUMN IF NOT EXISTS "calendar_id" varchar(8);
...
-- Idempotência da sync: uma ocorrência do feed = no máximo uma tarefa.
CREATE UNIQUE INDEX IF NOT EXISTS "tasks_calendar_uid_uidx"
	ON "tasks" ("calendar_id", "external_uid")
	WHERE "calendar_id" IS NOT NULL;

-- Lápides: evento apagado à mão não volta na próxima sync.
CREATE TABLE IF NOT EXISTS "calendar_ignored_events" (
	"calendar_id" varchar(8) NOT NULL REFERENCES "calendar_subscriptions"("id") ON DELETE CASCADE,
